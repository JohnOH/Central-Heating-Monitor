...
//                rf12_buf[4] = ~90;
                ...
                unsigned int currentTemp, targetTemp, lowestTemp, crc;
                switch (rf12_buf[1]) {
                    case 165: // Thermostat
                        currentTemp = ((rf12_buf[6] << 8) | rf12_buf[5]);
                        Serial.print(" Current="); Serial.print(currentTemp);
                        lowestTemp = ((rf12_buf[8] << 8) | rf12_buf[7]);
                        Serial.print(" Lowest="); Serial.print(lowestTemp);
                        targetTemp = ((rf12_buf[10] << 8) | rf12_buf[9]);
                    ...
                    case 166:   // OTO One Touch Override
                        ...
                        if (getOTO) {
                        ...
                        } else if ((addr == settings.addrOTO) && ((rf12_buf[5]) - rf12_buf[4] == settings.checkOTO)) {
                            Serial.print(" One Touch Override Matched");
                            Serial.println(", Relaying OTO");
                            ...
                            //
                            Serial.print(commandOTO[3]); printOneChar(':'); Serial.print(commandOTO[4]);
                            rf12_sleep(RF12_WAKEUP);
                            rf12_sendStart(0, &commandOTO, sizeof commandOTO);        // Forward the OTO command.
                            rf12_sendWait(1);                                         // Wait for transmission complete.
                            rf12_sleep(RF12_SLEEP);                                
