          ...
          if (rf12_buf[2] > 0) {
              ...
              switch (rf12_buf[3]) {
                  ...
                  case 10:
                      ...
                      break;
                  case 11:
                      settings.tempTrack = 100;                    // +/- 1 degree when testing temperatures
                      break;
                  case 12:
                      settings.tempTrack = 200;                    // +/- 2 degrees when testing temperatures
                      break;
 ...
 */
        ...
        if (!needSetback) {
            ...
            if (!setback) {
                rf12_sendStart(0, &setBack2, 5);                          // Issue a OTO to setback.
                rf12_sendWait(1);                                         // Wait for transmission complete.
        ...
        if (backCount > 9) {
            if (setback) {
                rf12_sendStart(0, &setBack2, 5);                          // Issue a OTO to refresh setback.
            } else {
...
//                rf12_buf[4] = ~90;
    ...
    if ((elapsed >= TEMPCHECK) || (commandResponse)) {
        ...
        if ((payload.BoilerFeed >= (settings.maxBoiler + settings.tempTrack)) || (payload.CentralHeatingReturn >= (settings.maxReturn + settings.tempTrack))) {
            // Serial.println("Above threshold");
            needSetback = true;
            payload.setBack = 1;
        } else if ((payload.BoilerFeed <= (settings.maxBoiler - settings.tempTrack)) || (payload.CentralHeatingReturn <= (settings.maxReturn - settings.tempTrack))) {
